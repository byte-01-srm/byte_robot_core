#!/usr/bin/env python3
"""
bound_gait_node_sid.py
════════════════════════════════════════════════════════════════════════════════
BYTE-01 Bound Gait — Interactive Terminal Controller
...
Commands:
  f          → bound forward  1 full cycle
  ff         → bound forward  continuously  (press y to stop)
  b          → bound backward 1 full cycle
  bb         → bound backward continuously  (press y to stop)
...
Bound front/back pairing:
  Pair A  (FL + FR) : phase offset 0.0   ← FRONT pair
  Pair B  (BL + BR) : phase offset 0.5   ← BACK  pair
  phase ∈ [0.0, 0.5) → swing
  phase ∈ [0.5, 1.0) → stance

  Effect: both front legs swing together while both back legs stance, then swap.
  Produces a pronk/bound-like motion. Pitches front-to-back.
"""
...
GAIT_FREQUENCY = 2       # Hz  — full cycle rate
...
#   Bound pairs front legs and back legs:
#     Front pair: FL + FR → offset 0.0
#     Back  pair: BL + BR → offset 0.5
# ─────────────────────────────────────────────────────────────────────────────
PHASE_OFFSET = {
    "fl": 0.0,   # ─┐ Pair A — FRONT
    "fr": 0.0,   # ─┘
...
def main():
    print("╔══════════════════════════════════════════════════╗")
    print("║     BYTE-01 Bound Gait Controller                ║")
    print("╠══════════════════════════════════════════════════╣")
    ...
    print(f"║  Pairing               = BOUND (FL+FR | BL+BR)  ║")
    print("╠══════════════════════════════════════════════════╣")
